...
def generate_hdf5(ftxt, output, fname, argument=False):
    ...
    for (imgPath, bbox, landmarkGt) in data:
        ...
        f_face = img[f_bbox.top:f_bbox.bottom+1,f_bbox.left:f_bbox.right+1] #截图

        ## data argument
        if argument and np.random.rand() > -1:
            ### flip
            face_flipped, landmark_flipped = flip(f_face, landmarkGt)  #图片翻转，数据增强，相应坐标也要发生变换
            face_flipped = cv2.resize(face_flipped, (39, 39))  #固定图片大小
            F_imgs.append(face_flipped.reshape((1, 39, 39)))   #caffe要求输入的格式  ['batch_size','channel','height','width']
